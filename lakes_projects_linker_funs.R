...
lakeprojects <- function(lakeID = c(), fpath = dbdir, dbname = db, tableNames = FALSE){
  l <- dbTableList(fpath = dbdir, dbname = db)
  
  # get a list of tables that link lakeID's to projectID's
  linkertab_names <- c() # create empty vector of linker table names
  u <- dbTable("UNITS")
...
projectlakes <- function(projectID = c(), fpath = dbdir, dbname = db, tableNames = FALSE){
  l <- dbTableList(fpath = dbdir, dbname = db)
